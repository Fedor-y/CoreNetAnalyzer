...
cd CoreNetAnalyzer/netScaner/main_directory
```

## 💻 Использование

### Вариант 1: Интерактивный режим (Аналитика в реальном времени)
Запустите скрипт в терминале. Сниффер начнет работу и фоновую запись логов. Для остановки и вывода красивого отчета нажмите `Ctrl+C`.
...
```
В этом режиме скрипт будет тихо собирать данные и писать их в файл `network_traffic.jsonl` в формате JSON Lines. Остановить процесс можно через команду `kill`.
